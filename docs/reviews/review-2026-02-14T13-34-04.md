...
- [x] L-1: `src/core/chronicle.ts:279-290,382-394` — **[Security/Goal]** `log()` untyped escape hatch passes fields directly without validation, sanitization, or prototype pollution protection. `sanitizeStrings: true` has no effect on `log()` calls. **Fixed:** Added `sanitizeLogFields()` call in both `log()` methods when `config.sanitizeStrings` is true.
- [x] L-2: `src/core/validation.ts:13` — **[Goal/Security]** ANSI escape regex only matches SGR sequences (ending in `m`). Cursor movement, screen clearing, and other CSI sequences pass through unsanitized. **Fixed:** Expanded regex to match all ANSI CSI sequences.
- [x] L-3: `src/cli/generator/docs-generator.ts:65` — **[Goal]** Table-of-contents anchor links don't lowercase the key, causing broken links for camelCase event keys in GitHub-flavored Markdown. **Fixed:** Added `.toLowerCase()` to anchor generation.
- [x] L-4: `src/cli/generator/docs-generator.ts:78-82` — **[Goal]** Standalone events filter only checks top-level group events, not nested sub-groups. Events in nested groups may appear duplicated in docs. **Fixed:** Added `collectAllGroupEventKeys()` helper that traverses nested sub-groups iteratively.
- [x] L-5: `src/core/chronicle.ts:197-198` — **[Goal]** `maxForkDepth` semantics are ambiguous. A JSDoc comment should clarify that depth N means N levels of nesting from root. **Fixed:** Added JSDoc to `maxForkDepth` property.
- [x] L-6: `src/core/events.ts:106` — **[Goal]** `EVENT_KEY_RE` allows camelCase but CLAUDE.md says "dotted lowercase". The regex and documentation disagree. **Fixed:** Updated CLAUDE.md to say "dotted camelCase" matching the regex and error messages.
- [x] L-7: `src/core/fields.ts:62` — **[YAGNI]** `field` alias for `t` is exported but only used in one test. Speculative API surface. **Accepted deviation:** Intentionally added as D-4 (HIGH DX item) to provide a non-colliding alias for i18n codebases.
- [x] L-8: `src/core/utils.ts:1-20` — **[YAGNI]** `utils.ts` contains a single function (`isSimpleValue`) with a single consumer (`context-store.ts`). Could be inlined. **Fixed:** Inlined `isSimpleValue` into `context-store.ts`, deleted `utils.ts`.
- [x] L-9: `src/cli/config-loader.ts:29`, `src/cli/parser/runtime-parser.ts:162` — **[Architecture/YAGNI]** tsx ESM loader registration/unregistration is duplicated in two files. Could extract a shared `withTsxLoader(fn)` utility. **Accepted deviation:** The two call sites have different error handling and return types. A shared utility would couple config loading to event parsing for minimal DRY benefit.
- [x] L-10: `src/core/chronicle.ts:540` — **[YAGNI]** Root chronicle's `currentCorrelationId` wraps `correlationIdGenerator` in an unnecessary closure: `() => correlationIdGenerator()` is identical to passing `correlationIdGenerator` directly. **Already resolved:** M-13 refactoring eliminated the closure — `correlationIdGenerator` is now passed directly.
- [x] L-11: `src/core/chronicle.ts:74-76` — **[YAGNI]** `ActiveCorrelationCounter` interface wraps a single `{ count: number }`. Could use inline type. **Fixed:** Inlined as `{ count: number }` at all usage sites, removed interface.
- [x] L-12: `src/index.ts:1` — **[Architecture]** Public barrel export includes `ChroniclerCliConfig` alongside core library types. CLI types should be separated via package.json exports map. **Accepted deviation:** Requires package.json `exports` map restructuring which is a breaking change best deferred to a major version.
- [x] L-13: Multiple files — **[Standards]** Loops lack documented upper bounds (Rule 8.1). For-of loops over `Object.entries()` in validation, context-store, events, etc. **Accepted deviation:** All loops iterate over finite `Object.entries()` results bounded by config limits (maxContextKeys, field definitions). Adding JSDoc upper-bound annotations to each would be noise.
- [x] L-14: `src/cli/config-loader.ts:38-47` — **[Security]** Loaded config object is not validated against a runtime schema. Default export is trusted as `ChroniclerCliConfig` without Zod/Valibot validation. **Accepted deviation:** The config file is user-authored and locally invoked. Adding Zod introduces a new runtime dependency for a 2-field interface. Existing checks (`!config`, `!config.eventsFile`) catch the critical cases.
