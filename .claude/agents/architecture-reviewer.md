...
The following check is **enforced by ESLint** and will be caught deterministically in CI. **Do NOT flag it** — it is already covered:

- Circular dependencies (Rule 10.3) — `import/no-cycle` (maxDepth: 5)

Focus your review **exclusively** on architectural judgment that static tools cannot enforce.
...
### Circular Dependencies (automated by `import/no-cycle` — review only for false negatives)

- [ ] Barrel exports (`index.ts`) not creating hidden circular deps that the linter might miss
- [ ] Check for indirect circular dependency chains that exceed maxDepth: 5
