...
- Unused variables/imports — `@typescript-eslint/no-unused-vars` (in recommended-type-checked), `noUnusedLocals`/`noUnusedParameters` in tsconfig

Focus your review **exclusively** on judgment-based YAGNI concerns that static tools cannot detect.
...
### Dead Code (unused variables/imports handled by ESLint — focus on architectural dead code)

- [ ] Unused exports (exported but never imported elsewhere — requires cross-file analysis)
- [ ] Commented-out code blocks
- [ ] Functions that are defined but never called (cross-module)
- [ ] Unreachable code after return/throw (structural, not syntactic)
